...
def cpncc(img, vertices_lst, tri):
    ...
    for i in range(len(vertices_lst)):
        vertices = vertices_lst[i]
        #addddeddd!
#        x = np.expand_dims(np.linspace(0,1,vertices.shape[1]),0)
#        y = np.expand_dims(np.linspace(1,0,vertices.shape[1]),0)
#        z = np.zeros((1,vertices.shape[1]))
#        z = np.concatenate((x,y,z),0)
        
        z=vertices[2,:]
        z_min, z_max = min(z), max(z)
        temp3 = (z - z_min) / (z_max - z_min)
        temp3=np.expand_dims(temp3,0)
        y=vertices[1,:]
        y_min, y_max = min(y), max(y)
        temp2 = (y - y_min) / (y_max - y_min)
        temp2=np.expand_dims(temp2,0)
        x=vertices[0,:]
        x_min, x_max = min(x), max(x)
        temp1 = (x - x_min) / (x_max - x_min)
        temp1=np.expand_dims(temp1,0)
        
        z = np.concatenate((temp1,temp2,temp3),0)
        pncc_img = crender_colors(vertices.T, tri.T, z.T, h, w, c)
